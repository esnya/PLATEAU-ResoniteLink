...
public static class CliArgumentsParser
{
    ...
    private static CliParseResult ParseBuild(string[] args)
    {
        ...
        PlateauImportSource source = ParseImportSourceInput(cityGmlSourceInput);
        PlateauImportSource? demTextureSource = string.IsNullOrWhiteSpace(orthoSourceInput)
            ? null
            : ParseImportSourceInput(orthoSourceInput);
    ...
    private static PlateauImportSource ParseImportSourceInput(string sourceInput)
    {
        ...
        return Uri.TryCreate(trimmedInput, UriKind.Absolute, out Uri? uri)
            && (string.Equals(uri.Scheme, Uri.UriSchemeHttp, StringComparison.OrdinalIgnoreCase)
                || string.Equals(uri.Scheme, Uri.UriSchemeHttps, StringComparison.OrdinalIgnoreCase))
            ? PlateauImportSource.Remote(uri)
            : PlateauImportSource.Local(trimmedInput);
    }
