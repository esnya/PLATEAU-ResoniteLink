...
internal sealed class ResoniteQueuedCityObjectSender(
    IResonitePreparedCityObjectFactory preparedCityObjectFactory,
    IResonitePreparedCityObjectImporter preparedCityObjectImporter) : IResoniteQueuedCityObjectSender
{
    private readonly IResonitePreparedCityObjectFactory preparedCityObjectFactory =
        preparedCityObjectFactory ?? throw new ArgumentNullException(nameof(preparedCityObjectFactory));
        ...
        CancellationToken processingCancellationToken)
    {
        using CancellationTokenSource linkedCancellation = CancellationTokenSource.CreateLinkedTokenSource(
            callerCancellationToken,
            processingCancellationToken);
        return await preparedCityObjectFactory.CreateAsync(
            state,
            routedClient,
            cityObject,
            diagnostics,
