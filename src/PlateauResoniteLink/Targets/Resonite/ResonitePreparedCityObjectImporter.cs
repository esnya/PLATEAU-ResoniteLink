...
internal sealed class ResonitePreparedCityObjectImporter(
    IResonitePreparedTextureUploader preparedTextureUploader,
    IResoniteGeometryAssetPlanner geometryAssetPlanner,
    IResoniteSceneMaterialPlanFactory sceneMaterialPlanFactory,
...
{
    private readonly IResonitePreparedTextureUploader preparedTextureUploader =
        preparedTextureUploader ?? throw new ArgumentNullException(nameof(preparedTextureUploader));
    ...
    private readonly IResoniteSceneMaterialPlanFactory sceneMaterialPlanFactory =
        sceneMaterialPlanFactory ?? throw new ArgumentNullException(nameof(sceneMaterialPlanFactory));
    private readonly IResoniteBatchEmissionPlanner batchEmissionPlanner =
        batchEmissionPlanner ?? throw new ArgumentNullException(nameof(batchEmissionPlanner));
    private readonly IResoniteSceneBatchEmitter batchEmitter =
        batchEmitter ?? throw new ArgumentNullException(nameof(batchEmitter));
        ...
        CancellationToken cancellationToken)
    {
        ...
        Task<UploadedTextureAssetSet> uploadedTextureAssetsTask = preparedTextureUploader.UploadAsync(
            state,
            routedClient,
            preparedCityObject,
        ...
        try
        {
            ...
            materialPlanningTask = sceneMaterialPlanFactory.CreateAsync(
                state,
                routedClient,
                cityObject,
                uploadedTextureAssets,
                step => ReportImportStep(progressReporter, cityObject, step),
                importStepCancellation.Token);
