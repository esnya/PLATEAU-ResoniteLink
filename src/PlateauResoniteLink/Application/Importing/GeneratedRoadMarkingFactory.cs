...
internal static class GeneratedRoadMarkingFactory
{
    public const double DefaultWidthMeters = 0.15;
    public const double DefaultSegmentLengthMeters = 5.0;

    private static readonly ColorRgba DefaultColor = new(1.0, 1.0, 1.0, 1.0);

    public static ParsedCityObject? TryCreate(
        ...
        LocalCartesian? cityObjectCartesian)
    {
        ...
        foreach (ParsedSurface surface in cityObject.Surfaces)
        {
            ...
            List<ParsedSurface> generatedSurfaces =
        ...
        LocalCartesian? cityObjectCartesian)
    {
        ...
        RoadSurfaceEdgeSelection edgePair = RoadSurfaceEdgeSelector.SelectPrimary(vertices, positions);
        if (edgePair.Length < 1.0 || edgePair.Width < 0.3)
        {
            return [];
        }

        double markingWidth = Math.Min(DefaultWidthMeters, edgePair.Width * 0.5);
        ...
        for (int segmentIndex = 0; segmentIndex < segmentCount; segmentIndex++)
        {
            ...
            GeodeticPoint side0Start = Lerp(edgePair.Side0[0], edgePair.Side0[1], startT);
            GeodeticPoint side0End = Lerp(edgePair.Side0[0], edgePair.Side0[1], endT);
            GeodeticPoint side1Start = Lerp(edgePair.Side1[0], edgePair.Side1[1], startT);
            GeodeticPoint side1End = Lerp(edgePair.Side1[0], edgePair.Side1[1], endT);
