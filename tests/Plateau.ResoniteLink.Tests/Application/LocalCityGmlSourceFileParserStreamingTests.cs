...
{
    [Fact]
    public async Task StreamParsedCityObjectsCoreAsyncYieldsBeforeReadingEntireFile()
    {
        string xml =
            """
        ...
        int secondCityObjectOffset = xml.IndexOf("<core:cityObjectMember>", xml.IndexOf("<core:cityObjectMember>", StringComparison.Ordinal) + 1, StringComparison.Ordinal);
        GateableDatasetContentSource datasetSource = new(bytes, secondCityObjectOffset);
        ...
        await using IAsyncEnumerator<LocalCityGmlResonitePlanBuilder.ParsedCityObject> enumerator =
            LocalCityGmlResonitePlanBuilder.StreamParsedCityObjectsCoreAsync(
                    sourceFile,
                    datasetSource,
                    Array.Empty<LocalCityGmlResonitePlanBuilder.MeshCodeArea>(),
