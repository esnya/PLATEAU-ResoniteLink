...
public sealed class ResoniteLinkSceneBuilderTests
{
    ...
    public async Task BuildAsyncImportsAssetsAndBuildsLiveComponents()
    {
        ...
        Assert.True(fakeClient.ImportedTexturePaths.Count >= 5);
        Assert.InRange(fakeClient.ImportedMeshes.Count, 1, scene.CityObjects.Count);
        ...
        string buildingFileSlotName = ResolveSharedScopeSlotName(buildingOne);
        string buildingLodSlotName = buildingOne.LodLevel.HasValue
            ...
            : "LOD0";
        string buildingAssetPrefix = $"PLATEAU tokyo23ku/Assets/{buildingFileSlotName}/";
        ...
        Assert.Contains(fakeClient.SlotPaths.Values, path => path.StartsWith(buildingAssetPrefix, StringComparison.Ordinal));
        Assert.Contains(
            fakeClient.SlotPaths.Values,
            path => path.StartsWith($"PLATEAU tokyo23ku/{buildingFileSlotName}/{buildingLodSlotName}/", StringComparison.Ordinal));
        ...
        AddComponent[] staticTextureRequests = fakeClient.AddedComponents
            ...
            .ToArray();
        Assert.True(staticTextureRequests.Length >= 4);
        ...
        Assert.Contains(
            ...
            path => string.Equals(
                ...
                || path.StartsWith("generated/lod2-atlas/53394525/", StringComparison.Ordinal));
        Assert.Contains(
            fakeClient.ImportedTexturePaths,
        ...
        Assert.Contains(
            ...
            rawImport => string.Equals(
                    ...
                    "udx/bldg/53394525/appearance/roof.png",
                    StringComparison.Ordinal)
                || (rawImport.Identity?.StartsWith("generated/lod2-atlas/53394525/", StringComparison.Ordinal) ?? false));
        ...
        Assert.DoesNotContain(
            fakeClient.SlotPaths.Values,
            path => path.StartsWith(buildingAssetPrefix, StringComparison.Ordinal)
                && path.Count(static c => c == '/') > 5);
        ...
        Component meshRenderer = Assert.Single(
            fakeClient.AddedComponents.Where(request =>
                    string.Equals(request.Data.ComponentType, "[FrooxEngine]FrooxEngine.MeshRenderer", StringComparison.Ordinal)
                    && fakeClient.SlotPaths[request.ContainerSlotId].StartsWith(
                        $"PLATEAU tokyo23ku/{buildingFileSlotName}/{buildingLodSlotName}/",
                        StringComparison.Ordinal))
                .Select(static request => request.Data));
        SyncList materials = Assert.IsType<SyncList>(meshRenderer.Members["Materials"]);
        Assert.NotEmpty(materials.Elements);
    ...
    public async Task BuildAsyncCreatesIndependentSlotsAndComponentsWithoutDuplicateAdds()
    {
        ...
        Assert.Contains(
            fakeClient.AddedSlots,
            request => request.Data.Name?.Value?.StartsWith("LOD", StringComparison.Ordinal) ?? false);
        Assert.DoesNotContain(
            fakeClient.AddedSlots
    ...
    public async Task BuildAsyncUsesDedicatedMaterialsForDatasetTexturedMaterials()
    {
        ...
        Assert.Contains(
            fakeClient.ImportedTexturePaths,
            static path => path.StartsWith("generated/lod2-atlas/53394525/", StringComparison.Ordinal));
        Assert.DoesNotContain(
            fakeClient.ImportedTexturePaths,
            path => string.Equals(path, textureOne, StringComparison.Ordinal));
        Assert.DoesNotContain(
            fakeClient.ImportedTexturePaths,
            path => string.Equals(path, textureTwo, StringComparison.Ordinal));
    }
    ...
    public async Task BuildAsyncDoesNotUseMaterialPropertyBlocksForMixedMaterialStrategies()
    {
        ...
        Component[] meshRenderers = fakeClient.AddedComponents.Where(request =>
                ...
                && slotPath.Contains("/LOD2/", StringComparison.Ordinal)
        ...
        Assert.DoesNotContain(
            fakeClient.ImportedTexturePaths,
            path => string.Equals(path, datasetTexture, StringComparison.Ordinal));

        Component mixedRenderer = Assert.Single(meshRenderers);
        SyncList materials = Assert.IsType<SyncList>(mixedRenderer.Members["Materials"]);
    ...
    [Fact]
    public async Task CompleteAsyncAppliesDatasetLicenseAfterTerrainTextureGenerationDrains()
    {
        const string overlayTexturePath = "terrain://dem/plateau-ortho/test";
        TerrainTextureOverlay overlay = new(
            TexturePath: overlayTexturePath,
            PackageName: "dem",
            UrlTemplate: "https://tiles.example/{z}/{x}/{y}.png",
            ZoomLevel: 18,
            GeographicBounds: new GeographicRectangle(35.0, 35.001, 139.0, 139.001),
            MaxTextureSize: 256);
        CapturedResoniteScene scene = new(
            new ResoniteConstructionMetadata(
                SchemaVersion: "3.0",
                WorldName: "PLATEAU delayed-license-test",
                Request: new PlateauImportRequest(
                    Dataset: "delayed-license-test",
                    MeshCode: "53394525",
                    SourceKind: DatasetSourceKind.Local,
                    LocalSourcePath: TestData.GetFixturePath("LocalPlateauDataset"),
                    ServerUri: null),
                SourceDataset: new PlateauSourceDataset(
                    PackageNames: ["tran", "dem"],
            ...
            [
                new ResoniteConstructionCityObject(
                    SlotKey: "terrain-license-probe",
                    DisplayName: "Terrain License Probe",
                    PackageName: "tran",
                    ActualMeshCode: "53394525",
                    LodLevel: 1,
                    Transform: new ResoniteTransform(new ResoniteFloat3(0.0, 0.0, 0.0)),
                    Mesh: CreateTriangleMesh("terrain-license-probe-material"),
                    Materials:
                    [
                        new ResoniteMaterialBinding(
                            MaterialKey: "terrain-license-probe-material",
                            BaseColor: new ResoniteColor(1.0, 1.0, 1.0, 1.0),
                            MaterialType: ResoniteMaterialType.Standard,
                            TexturePath: overlayTexturePath,
                            TextureSourceKind: ResoniteTextureSourceKind.Dataset,
                    ...
                    SourceObjectKey: "terrain-license-probe-source"),
            ]);

        using FakeResoniteLinkClient fakeClient = new();
        DelayedLicenseAwareTerrainTextureAssetGenerator terrainTextureAssetGenerator = new();
        await using ResoniteLinkSceneBuilder builder = new(
            new Uri("ws://localhost:12345/"),
            1,
            ResoniteLinkSendDiagnostics.Disabled,
        ...
        await terrainTextureAssetGenerator.EnsureStarted.Task.WaitAsync(TimeSpan.FromSeconds(3));
        await Task.Delay(150);
        Assert.Equal(0, terrainTextureAssetGenerator.ResolveCallCount);

        terrainTextureAssetGenerator.AllowEnsureCompletion.TrySetResult();
        IReadOnlyList<string> destinations = await completionTask.WaitAsync(TimeSpan.FromSeconds(5));

        Assert.Single(destinations);
        Assert.Equal(1, terrainTextureAssetGenerator.ResolveCallCount);
        Assert.True(terrainTextureAssetGenerator.EnsureCompletedBeforeResolve);
    }
    ...
    public async Task PrepareMaterialTargetsAwaitsReferencedCommonMaterialFamilyBeforeMaterialCreation()
    {
        ...
        Assert.False(client.MeshRendererAdded.Task.IsCompleted);
    ...
    public async Task BuildAsyncPlacesHeightMapDisplacementTextureOnDedicatedAssetSlot()
    {
        ...
        string cityObjectFileSlot = ResolveSharedScopeSlotName(Assert.Single(scene.CityObjects));
        Assert.StartsWith(
            $"PLATEAU tokyo23ku/Assets/{cityObjectFileSlot}/LOD0/HeightMap Overlay Test/",
    ...
    public async Task BuildAsyncDoesNotPlaceDatasetScopedMaterialWithoutTextureInCommonAssets()
    {
        ...
        string cityObjectFileSlot = ResolveSharedScopeSlotName(Assert.Single(scene.CityObjects));
        Assert.StartsWith(
            $"PLATEAU tokyo23ku/Assets/{cityObjectFileSlot}/LOD1/",
            materialPath,
            StringComparison.Ordinal);
    ...
    public async Task BuildAsyncCreatesConcreteAnchorRootWhenRequestMeshCodeIsRegex()
    {
        ...
        Assert.Equal(
            ["53394525", "Assets", "bldg_53394525", "dem_shared"],
            (await GetDirectChildNamesAsync(fakeClient, datasetSlot.ID)).OrderBy(static name => name, StringComparer.Ordinal).ToArray());
    ...
    public async Task BuildAsyncPlacesRegexParentMeshRootFromAnchorOffset()
    {
        ...
        Slot anchorMeshRootSlot = fakeClient.SlotsById[Assert.Single(
            fakeClient.SlotPaths,
            static entry => string.Equals(entry.Value, "PLATEAU tokyo23ku/bldg_53394525", StringComparison.Ordinal)).Key];
        Slot parentMeshRootSlot = fakeClient.SlotsById[Assert.Single(
            fakeClient.SlotPaths,
            static entry => string.Equals(entry.Value, "PLATEAU tokyo23ku/dem_shared", StringComparison.Ordinal)).Key];
        Field_float3? anchorMeshRootPosition = anchorMeshRootSlot.Position as Field_float3;
        Field_float3? parentMeshRootPosition = parentMeshRootSlot.Position as Field_float3;
        ...
        Assert.Equal(0.0f, anchorMeshRootPosition?.Value.x ?? 0.0f, precision: 4);
        Assert.Equal(0.0f, anchorMeshRootPosition?.Value.y ?? 0.0f, precision: 4);
        Assert.Equal(0.0f, anchorMeshRootPosition?.Value.z ?? 0.0f, precision: 4);
        Assert.Equal(0.0f, parentMeshRootPosition?.Value.x ?? 0.0f, precision: 4);
        Assert.Equal(0.0f, parentMeshRootPosition?.Value.y ?? 0.0f, precision: 4);
        Assert.Equal(0.0f, parentMeshRootPosition?.Value.z ?? 0.0f, precision: 4);
    }
    ...
    public async Task BuildAsyncCanDistributeImportsAcrossMultipleConnections()
    {
        ...
        Assert.Contains(clients, static client => client.BatchMutationCount > 0);
    ...
    [Fact]
    public async Task BuildAsyncCreatesCityObjectWhenExistingLodSlotAlreadyContainsDuplicateIdentityTags()
    {
        ...
        Assert.DoesNotContain(
            ...
            message => message.Contains(
                ...
                StringComparison.Ordinal));
        Assert.Contains(progressMessages, static message => message.Contains("Sent city object", StringComparison.Ordinal));
        Assert.Contains(
            fakeClient.SlotsById.Values,
            slot => string.Equals(slot.Tag?.Value, identityTag, StringComparison.Ordinal));
    }
    ...
    public async Task BuildAsyncReusesPreferredExistingMeshRootWhenDatasetRootContainsDuplicateNamedChildren()
    {
        ...
        string duplicateMeshRootId = AddSeedSlot(session, datasetRootId, building.ActualMeshCode);
        _ = duplicateMeshRootId;
        AddSeedSlot(session, datasetRootId, building.ActualMeshCode);
        int seededDuplicateMeshRootCount = 2;
        ...
        IReadOnlyList<string> destinations = await RunBuilderAsync(
            ...
            new CapturedResoniteScene(
                scene.Metadata,
                [building]));

        Assert.Single(destinations);
        Assert.True(session.SlotsById.Values.Count(slot =>
            string.Equals(slot.Name?.Value, building.ActualMeshCode, StringComparison.Ordinal)
                && string.Equals(slot.Parent?.TargetID, datasetRootId, StringComparison.Ordinal))
            >= seededDuplicateMeshRootCount);
    }
    ...
    public async Task BeginAsyncReusesExistingDatasetRootAssetsAndCommonBySlotNameAcrossRuns()
    {
        ...
        Assert.Contains("PLATEAU tokyo23ku", session.SlotPaths.Values);
        Assert.Contains("PLATEAU tokyo23ku/Assets", session.SlotPaths.Values);
        Assert.Contains("PLATEAU tokyo23ku/Assets/Common", session.SlotPaths.Values);
    }

    [Fact]
    public async Task BuildAsyncFreshRunOnlyReadsWithinDatasetAndAssetScope()
    {
        string fixturePath = TestData.GetFixturePath("LocalPlateauDataset");
        CapturedResoniteScene scene = LoadScene(
            new PlateauImportRequest(
                ...
                ServerUri: null));
        FakeResoniteLinkSession session = new();
        using ReadScopeEnforcedClient fakeClient = new(session, "tokyo23ku");
        ...
        Assert.Empty(fakeClient.DisallowedGetSlotPaths);
        Assert.Empty(fakeClient.GetComponentCalls);
    }

    [Fact]
    public async Task BuildAsyncGetSlotReadsStayWithinDatasetAndAssetScope()
    {
        string fixturePath = TestData.GetFixturePath("LocalPlateauDataset");
        CapturedResoniteScene scene = LoadScene(
            new PlateauImportRequest(
        ...
        string assetsRootId = AddSeedSlot(session, datasetRootId, "Assets");
        _ = AddSeedSlot(session, assetsRootId, "Common");
    ...
    [Fact]
    public async Task ProcessCityObjectAsyncAndCompleteAsyncDoNotIssueGetReads()
    {
        CapturedResoniteScene scene = CreateExactParentMeshScene();
        FakeResoniteLinkSession session = new();
        using NoGetReadsAfterBeginClient fakeClient = new(session);
        await using ResoniteLinkSceneBuilder builder = new(
            new Uri("ws://localhost:12345/"),
            1,
            ResoniteLinkSendDiagnostics.Disabled,
        ...
        await builder.BeginAsync(scene.Metadata, Path.GetTempPath());
        fakeClient.DisallowReads();
    ...
    [Fact]
    public async Task BuildAsyncCreatesExpectedMeshFileLodHierarchyWithoutLowerLevelReads()
    {
        CapturedResoniteScene scene = CreateExactParentMeshScene();
        FakeResoniteLinkSession session = new();
        using ReadScopeEnforcedClient fakeClient = new(session, "tokyo23ku");
        ...
        string lodSlotName = FormatLodSlotName(cityObject.LodLevel);
        string fileSlotName = ResolveSharedScopeSlotName(cityObject);
        string cityObjectPath = $"PLATEAU tokyo23ku/{fileSlotName}/{lodSlotName}/{cityObject.DisplayName}";
        string meshRootPath = $"PLATEAU tokyo23ku/{fileSlotName}/{lodSlotName}";
        string assetMeshPath = $"PLATEAU tokyo23ku/Assets/{fileSlotName}/{lodSlotName}";

        KeyValuePair<string, Slot> cityObjectSlotEntry = Assert.Single(
            fakeClient.SlotsById,
            slotEntry => string.Equals(fakeClient.SlotPaths[slotEntry.Key], cityObjectPath, StringComparison.Ordinal));
        Slot cityObjectSlot = cityObjectSlotEntry.Value;
        Assert.Equal(cityObjectPath, fakeClient.SlotPaths[cityObjectSlot.ID]);

        Slot lodSlot = fakeClient.SlotsById[Assert.IsType<string>(cityObjectSlot.Parent?.TargetID)];
        Assert.Equal(lodSlotName, lodSlot.Name?.Value);
        Assert.Equal(meshRootPath, fakeClient.SlotPaths[lodSlot.ID]);

        Slot fileSlot = fakeClient.SlotsById[Assert.IsType<string>(lodSlot.Parent?.TargetID)];
        Assert.Equal(fileSlotName, fileSlot.Name?.Value);
        Assert.Equal($"PLATEAU tokyo23ku/{fileSlotName}", fakeClient.SlotPaths[fileSlot.ID]);

        Slot meshRootSlot = fakeClient.SlotsById[Assert.IsType<string>(fileSlot.Parent?.TargetID)];
        Assert.Equal("PLATEAU tokyo23ku", meshRootSlot.Name?.Value);
        Assert.Equal("PLATEAU tokyo23ku", fakeClient.SlotPaths[meshRootSlot.ID]);

        Assert.Contains(
            fakeClient.SlotPaths.Values,
            path => path.StartsWith($"{assetMeshPath}/", StringComparison.Ordinal) || string.Equals(path, assetMeshPath, StringComparison.Ordinal));

        if (!string.Equals(fileSlotName, cityObject.PackageName, StringComparison.Ordinal))
        {
            Assert.NotEqual(
                $"PLATEAU tokyo23ku/{cityObject.ActualMeshCode}/{cityObject.PackageName}/{lodSlotName}",
                fakeClient.SlotPaths[lodSlot.ID]);
            Assert.DoesNotContain(
                fakeClient.SlotPaths.Values,
                path => path.Equals(
                    $"PLATEAU tokyo23ku/{cityObject.ActualMeshCode}/{cityObject.PackageName}/{lodSlotName}/{cityObject.DisplayName}",
                    StringComparison.Ordinal));
        }
    }
    ...
    public async Task BuildAsyncSkipsFailedCityObjectsAndContinues()
    {
        ...
        Assert.Contains(
            ...
            static message => message.Contains(
                "[live][info] Starting setup slot bootstrap: dataset root, assets root, common assets root, and anchor slot.",
                StringComparison.Ordinal));
        Assert.Contains(
            progressMessages,
    ...
    public async Task BuildAsyncDoesNotLeavePresentationSlotWhenGeometryImportFails()
    {
        ...
        IReadOnlyList<string> destinations = await RunBuilderAsync(builder, scene);
        Assert.Single(destinations);
        string buildingOneFileSlotName = ResolveSharedScopeSlotName(
            Assert.Single(scene.CityObjects, static cityObject => string.Equals(cityObject.DisplayName, "Building One", StringComparison.Ordinal)));
        Assert.DoesNotContain(
            session.SlotPaths.Values,
            path => string.Equals(
                path,
                $"PLATEAU tokyo23ku/{buildingOneFileSlotName}/LOD2/Building One",
                StringComparison.Ordinal));
    }
    ...
    public async Task BuildAsyncUsesResponseCanonicalIdsWhenCreateResponsesDiffer()
    {
        ...
        Assert.True(client.BatchMutationCount >= scene.CityObjects.Count);
    ...
    public async Task BuildAsyncIgnoresSameNameSiblingsDuringCreateCanonicalization()
    {
        ...
        Assert.Equal(0, client.RejectedAliasMutationCount);
        Assert.True(client.BatchMutationCount > 0);
    }
    ...
    public async Task BuildAsyncUsesOneDataModelBatchPerCityObject()
    {
        ...
        Assert.Single(destinations);
        List<IReadOnlyList<DataModelOperation>> cityObjectBatches = client.Batches
            .Where(batch => batch.OfType<AddComponent>().Any(static operation =>
                string.Equals(operation.Data.ComponentType, "[FrooxEngine]FrooxEngine.MeshRenderer", StringComparison.Ordinal)))
            .Cast<IReadOnlyList<DataModelOperation>>()
            .ToList();
        Assert.Equal(scene.CityObjects.Count, cityObjectBatches.Count);
        Assert.All(cityObjectBatches, batch =>
        {
            Assert.Single(
    ...
    public async Task BuildAsyncUsesGloballyUniqueBatchLocalIdsAcrossParallelWorkers()
    {
        ...
        Assert.Single(destinations);
        Assert.True(session.Batches.Count >= scene.CityObjects.Count);
    }
    ...
    [Fact]
    public async Task BuildAsyncCreatesAdditionalMeshRootWhenDuplicateNamedChildrenAlreadyExist()
    {
        FakeResoniteLinkSession session = new();
        string datasetRootId = AddSeedSlot(session, "Root", "PLATEAU tokyo23ku");
    ...
    [Fact]
    public async Task BeginAsyncFreshDatasetBootstrapCreatesSlotsInSingleBatch()
    {
        string fixturePath = TestData.GetFixturePath("LocalPlateauDataset");
        CapturedResoniteScene scene = LoadScene(
            new PlateauImportRequest(
        ...
        DataModelOperation[] bootstrapBatch = Assert.Single(fakeClient.Batches).ToArray();
        List<AddSlot> bootstrapSlotOperations = bootstrapBatch.OfType<AddSlot>().ToList();

        string completionMeshCode = ResoniteSourceMeshCodeAnchor.ResolveCompletionMeshCode(scene.Metadata);
        Assert.True(bootstrapSlotOperations.Count > 4);
        Assert.Equal("PLATEAU tokyo23ku", bootstrapSlotOperations[0].Data.Name?.Value);
        Assert.Equal("Assets", bootstrapSlotOperations[1].Data.Name?.Value);
        Assert.Equal("Common", bootstrapSlotOperations[2].Data.Name?.Value);
        Assert.Equal(completionMeshCode, bootstrapSlotOperations[3].Data.Name?.Value);
        Assert.Contains(
            bootstrapSlotOperations.Skip(4),
            operation => operation.Data.Parent?.TargetID == bootstrapSlotOperations[2].Data.ID);

        Assert.Contains("PLATEAU tokyo23ku", fakeClient.SlotPaths.Values);
        Assert.Contains("PLATEAU tokyo23ku/Assets", fakeClient.SlotPaths.Values);
        Assert.Contains("PLATEAU tokyo23ku/Assets/Common", fakeClient.SlotPaths.Values);
        Assert.Contains($"PLATEAU tokyo23ku/{completionMeshCode}", fakeClient.SlotPaths.Values);
    }

    [Fact]
    public async Task BeginAsyncExistingDatasetRootPathCreatesMissingSharedSetupInSingleBatch()
    {
        string fixturePath = TestData.GetFixturePath("LocalPlateauDataset");
        CapturedResoniteScene scene = LoadScene(
            new PlateauImportRequest(
        ...
        Assert.Equal(1, fakeClient.BatchMutationCount);
        Assert.Single(fakeClient.Batches);
        Assert.DoesNotContain(
            fakeClient.AddedSlots,
            static request => string.Equals(request.Data.Name?.Value, "PLATEAU tokyo23ku", StringComparison.Ordinal));
    }
    ...
    [Fact]
    public async Task BeginAsyncBatchesInitialBootstrapSlotCreation()
    {
        string fixturePath = TestData.GetFixturePath("LocalPlateauDataset");
        CapturedResoniteScene scene = LoadScene(
            new PlateauImportRequest(
        ...
        IReadOnlyList<DataModelOperation> bootstrapBatch = Assert.Single(fakeClient.Batches);
        AddSlot[] bootstrapSlots = bootstrapBatch.OfType<AddSlot>().ToArray();
        Assert.True(bootstrapSlots.Length > 4);
        Assert.Equal("PLATEAU tokyo23ku", bootstrapSlots[0].Data.Name?.Value);
        Assert.Equal("Assets", bootstrapSlots[1].Data.Name?.Value);
        Assert.Equal("Common", bootstrapSlots[2].Data.Name?.Value);
        Assert.Equal("53394525", bootstrapSlots[3].Data.Name?.Value);
        Assert.Equal("Root", bootstrapSlots[0].Data.Parent?.TargetID);
        Assert.Equal(bootstrapSlots[0].Data.ID, bootstrapSlots[1].Data.Parent?.TargetID);
        Assert.Equal(bootstrapSlots[1].Data.ID, bootstrapSlots[2].Data.Parent?.TargetID);
        Assert.Equal(bootstrapSlots[0].Data.ID, bootstrapSlots[3].Data.Parent?.TargetID);
        Assert.True(fakeClient.AddedSlots.Count > 4);
    }
    ...
    private sealed class FamilyAwareMaterialClient : IResoniteLinkClient
    {
        ...
        public Task<BatchResponse> RunDataModelOperationBatchAsync(
            ...
            CancellationToken cancellationToken)
        {
            if (operations.OfType<AddComponent>().Any(operation => IsCommonMaterialForDelayedFamily(operation.ContainerSlotId, operation.Data.ComponentType))
                || (delayedCommonMaterialFamily is not null
                    && operations.OfType<AddSlot>().Any(operation =>
                        operation.Data.Name?.Value?.Contains(
                            string.Create(CultureInfo.InvariantCulture, $"_{delayedCommonMaterialFamily}_"),
                            StringComparison.Ordinal) == true)))
            {
                DelayedCommonMaterialCreationStarted.TrySetResult(true);
                return WaitForDelayedCommonMaterialBatchAsync(operations, cancellationToken);
            }
        ...
        private async Task<BatchResponse> WaitForDelayedCommonMaterialBatchAsync(
            IReadOnlyList<DataModelOperation> operations,
            CancellationToken cancellationToken)
        {
            await AllowDelayedCommonMaterialCreation.Task.WaitAsync(cancellationToken);
    ...
    private sealed class ReadScopeEnforcedClient : IResoniteLinkClient
    {
        private readonly FakeResoniteLinkClient inner;
        private readonly string datasetRoot;
        private readonly List<string> disallowedGetSlotPaths = [];
        private readonly List<string> getComponentCalls = [];
        private readonly object getSlotGate = new();

        public ReadScopeEnforcedClient(FakeResoniteLinkSession session, string dataset)
        {
            inner = new FakeResoniteLinkClient(session);
            datasetRoot = string.Create(CultureInfo.InvariantCulture, $"PLATEAU {dataset}");
        }

        public IReadOnlyList<string> DisallowedGetSlotPaths
        {
            get
            {
                lock (getSlotGate)
                {
                    return [.. disallowedGetSlotPaths];
                }
            }
        }
        ...
        public IReadOnlyList<string> GetComponentCalls
        {
            get
            {
                lock (getSlotGate)
                {
                    return [.. getComponentCalls];
                }
            }
        }
        ...
        public Task<Component?> GetComponentAsync(string componentId, CancellationToken cancellationToken)
        {
            lock (getSlotGate)
            {
                getComponentCalls.Add(componentId);
            }
        ...
        {
            Slot? slot = await inner.GetSlotAsync(slotId, depth, cancellationToken);
            if (string.Equals(slotId, "Root", StringComparison.Ordinal))
            {
                return slot;
            }

            string slotPath = ResolveSlotPath(slotId, slot);
            if (!IsAllowedReadScope(slotPath))
            {
                lock (getSlotGate)
                {
                    disallowedGetSlotPaths.Add(slotPath);
                }
            }
        ...
        private string ResolveSlotPath(string slotId, Slot? slot)
        {
            if (inner.SlotPaths.TryGetValue(slotId, out string? slotPath))
            {
                return slotPath;
            }
            ...
            if (slot.Name is null || string.IsNullOrWhiteSpace(slot.Name.Value))
            {
                return slotId;
            }
            ...
            if (
                ...
                && inner.SlotPaths.TryGetValue(slot.Parent.TargetID, out string? parentPath)
                && !string.IsNullOrWhiteSpace(parentPath))
            {
                return $"{parentPath}/{slot.Name.Value}";
            }

            return slot.Name.Value;
        }

        private bool IsAllowedReadScope(string slotPath)
        {
            if (string.Equals(slotPath, datasetRoot, StringComparison.Ordinal))
            {
                return true;
            }

            if (!slotPath.StartsWith($"{datasetRoot}/", StringComparison.Ordinal))
            {
                return false;
            }

            string suffix = slotPath.Substring(datasetRoot.Length + 1);
            ...
            {
                1 => true,
                2 => string.Equals(segments[0], "Assets", StringComparison.Ordinal),
                3 => string.Equals(segments[0], "Assets", StringComparison.Ordinal)
                    && string.Equals(segments[1], "Common", StringComparison.Ordinal),
                _ => false,
            };
        }
    }

    private sealed class NoGetReadsAfterBeginClient(FakeResoniteLinkSession session) : IResoniteLinkClient
    {
        private readonly FakeResoniteLinkClient inner = new(session);
        private int readsAllowed = 1;
        ...
        public void DisallowReads()
        {
            Interlocked.Exchange(ref readsAllowed, 0);
        }
        ...
        public Task<Component?> GetComponentAsync(string componentId, CancellationToken cancellationToken)
        {
            ThrowIfReadsDisallowed($"GetComponentAsync({componentId})");
            return inner.GetComponentAsync(componentId, cancellationToken);
        }

        public Task<Slot?> GetSlotAsync(string slotId, int depth, CancellationToken cancellationToken)
        {
            ThrowIfReadsDisallowed($"GetSlotAsync({slotId}, {depth})");
            return inner.GetSlotAsync(slotId, depth, cancellationToken);
        }
        ...
        private void ThrowIfReadsDisallowed(string operation)
        {
            if (Volatile.Read(ref readsAllowed) == 0)
            {
                throw new InvalidOperationException($"Unexpected post-initialization read: {operation}");
            }
        }
    }
    ...
    {
        private int ensureCompleted;

        public TaskCompletionSource EnsureStarted { get; } = new(TaskCreationOptions.RunContinuationsAsynchronously);

        public TaskCompletionSource AllowEnsureCompletion { get; } = new(TaskCreationOptions.RunContinuationsAsynchronously);

        public int ResolveCallCount { get; private set; }

        public bool EnsureCompletedBeforeResolve { get; private set; }
            ...
            CancellationToken cancellationToken)
        {
            ...
            await AllowEnsureCompletion.Task.WaitAsync(cancellationToken);
            Interlocked.Exchange(ref ensureCompleted, 1);
    ...
    private static BatchResponse CreateSyntheticBatchResponse(
        IReadOnlyList<DataModelOperation> operations,
        Func<AddSlot, string> resolveSlotId,
        Func<AddComponent, string> resolveComponentId)
    {
        List<Response> responses = [];
        foreach (DataModelOperation operation in operations)
        {
            switch (operation)
            {
                case AddSlot addSlot:
                    ...
                    {
                        Success = true,
                        EntityId = resolveSlotId(addSlot),
                        SourceMessageID = addSlot.MessageID,
                    });
                    break;
                case AddComponent addComponent:
                    ...
                    {
                        Success = true,
                        EntityId = resolveComponentId(addComponent),
                        SourceMessageID = addComponent.MessageID,
                    });
                    break;
                case UpdateComponent updateComponent:
                    ...
                    break;
                case GetComponent getComponent:
                    responses.Add(new ComponentData
                    {
                        Success = true,
                        Data = null,
                        SourceMessageID = getComponent.MessageID,
                    });
                    break;
                default:
                    throw new NotSupportedException($"Unsupported synthetic batch operation '{operation.GetType().Name}'.");
            }
        }
    ...
    private sealed class BlockingMeshFailingTextureClient : IResoniteLinkClient
    {
        private int nextComponentId;
        private int nextSlotId;
        private readonly Dictionary<string, Slot> slotsById = [];
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            string slotId = $"srv_slot_{Interlocked.Increment(ref nextSlotId)}";
            slotsById[slotId] = new Slot
            {
                ID = slotId,
                Name = new Field_string
                {
                    Value = request.Data?.Name?.Value ?? slotId,
                },
            };
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            return Task.FromResult(CreateSyntheticBatchResponse(
                operations,
                addSlot =>
                {
                    ...
                    slotsById[slotId] = new Slot
                    {
                        ...
                        Name = new Field_string
                        {
                            Value = addSlot.Data?.Name?.Value ?? slotId,
                        },
                    };
                    return slotId;
                },
                addComponent => string.IsNullOrWhiteSpace(addComponent.Data?.ID)
                    ? $"srv_component_{Interlocked.Increment(ref nextComponentId)}"
                    : addComponent.Data.ID));
        }
        ...
        public Task<Slot?> GetSlotAsync(string slotId, int depth, CancellationToken cancellationToken)
        {
            ...
            slotsById.TryGetValue(slotId, out Slot? slot);
            return Task.FromResult<Slot?>(slot);
        }
    ...
    private sealed class BlockingBootstrapClient : IResoniteLinkClient
    {
        public TaskCompletionSource AddSlotStarted { get; } = new(TaskCreationOptions.RunContinuationsAsynchronously);

        public TaskCompletionSource AddSlotCanceled { get; } = new(TaskCreationOptions.RunContinuationsAsynchronously);
        ...
        public async Task<string> AddSlotAsync(AddSlot request, CancellationToken cancellationToken)
        {
            ...
            {
                AddSlotCanceled.TrySetResult();
                throw;
            }

            return "srv_slot_blocked";
        }
        ...
        public Task<BatchResponse> RunDataModelOperationBatchAsync(
            ...
            CancellationToken cancellationToken)
        {
            return WaitForBlockedBootstrapBatchAsync(cancellationToken);
        }

        private async Task<BatchResponse> WaitForBlockedBootstrapBatchAsync(CancellationToken cancellationToken)
        {
            AddSlotStarted.TrySetResult();
            try
            {
    ...
    private sealed class BlockingPeerTextureFailureClient(string failingTextureIdentitySuffix, string blockingTextureIdentitySuffix) : IResoniteLinkClient
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            return Task.FromResult(CreateSyntheticBatchResponse(
                operations,
                addSlot => string.IsNullOrWhiteSpace(addSlot.Data?.ID)
                    ? $"srv_slot_{Interlocked.Increment(ref nextSlotId)}"
                    : addSlot.Data.ID,
                addComponent => string.IsNullOrWhiteSpace(addComponent.Data?.ID)
                    ? $"srv_component_{Interlocked.Increment(ref nextComponentId)}"
    ...
    private sealed class DelayedConnectClient : IResoniteLinkClient
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            return Task.FromResult(CreateSyntheticBatchResponse(
                operations,
                addSlot => addSlot.Data.ID,
                addComponent => addComponent.Data.ID));
        }
    ...
    private sealed class NonCancelableBlockingResoniteLinkClient : IResoniteLinkClient
    {
        ...
        private Slot CloneSlot(Slot source, int depth)
        {
            Slot clone = new()
            {
                ...
                Tag = source.Tag is null ? null : new Field_string
                {
    ...
    private static string ResolveSharedScopeSlotName(ResoniteConstructionCityObject cityObject)
    {
        ...
        return new CapturedResoniteScene(
            ...
            [
                new ResoniteConstructionCityObject(
                    ...
                    SourceObjectKey: "udx/dem/533945/plateau_tokyo23ku_dem_533945.gml_dem_shared_parent",
                    SourceUnitKey: "unit-53394525-dem_shared_parent"),
            ]);
    }
