...
public sealed class LocalCityGmlObjectProjectionTests
{
    ...
    public void GeneratedFacadeUvProjection_UsesFloorUnitsForBuildingWalls()
    {
        ...
        SurfaceGeneratedUvProjection projection = CreateSurfaceGeneratedUvProjectionForTest(wallSurface, "bldg", origin, cartesian);
        Float2 uvOrigin = projection.CreateUv(origin, origin, cartesian);
        Float2 uvHorizontal = projection.CreateUv(wallVertices[1], origin, cartesian);
        Float2 uvVertical = projection.CreateUv(wallVertices[3], origin, cartesian);
    ...
    public void GeneratedFacadeUvProjection_AlignsVerticalPhaseToBuildingBottomAndTop()
    {
        ...
        SurfaceGeneratedUvProjection projection = CreateSurfaceGeneratedUvProjectionForTest(
            wallSurface,
            "bldg",
            origin,
            cartesian,
            ...
            floorCount: 2);
        Float2 uvBottom = projection.CreateUv(origin, origin, cartesian);
        Float2 uvTop = projection.CreateUv(wallVertices[3], origin, cartesian);
    ...
    public void CommonMaterialBindingEnumeratorPrecreatesCommonFacadeForTexturelessVerticalBuildingSurfaces(
        int semanticValue)
    {
        ...
        MaterialBinding[] materialBindings = CreateSharedCommonMaterialBindings(
            cityObject,
            origin,
            cartesian);
    ...
    [InlineData((int)ParsedSurfaceSemantic.Ground)]
    public void CommonMaterialBindingEnumeratorDoesNotGenerateFacadeMaterialForRoofOrGround(
        int semanticValue)
    {
        ParsedSurfaceSemantic semantic = (ParsedSurfaceSemantic)semanticValue;
        CoordinateReferenceSystem referenceSystem = CoordinateReferenceSystem.Parse("http://www.opengis.net/def/crs/EPSG/0/6697");
    ...
    public void ProjectCityObjectCullsBottomBandBuildingSurfacesBySemanticOrDownwardLod1Face()
    {
        ...
        HashSet<string> culledSurfaceIds = GetBottomBandCulledSurfaceIdsForTest(
            "bldg",
    ...
    [Fact]
    public void CommonMaterialBindingEnumeratorExcludesCulledBottomBandBuildingSurface()
    {
        CoordinateReferenceSystem referenceSystem = CoordinateReferenceSystem.Parse("http://www.opengis.net/def/crs/EPSG/0/6697");
        GeodeticPoint origin = new(35.0, 139.0, 0.0);
        GeographicLib.LocalCartesian cartesian = new(
    ...
    public void DemTerrainDynamicModeKeepsMaterialBindingsCompatibleWithStaticMesh()
    {
        ...
        ImportedCityObject projected = ProjectTerrainMeshModeForTest(
            cityObject,
            origin,
            new PlateauImportRequest(
                Dataset: "tokyo23ku",
    ...
    [Fact]
    public void DemTerrainGridChunkBoundaryAlignerAveragesSharedSamplesForPartialOverlapWithDifferentResolution()
    {
        ...
        ImportedCityObject[] aligned = AlignTerrainGridChunkBoundaries([left, right]);
    ...
    private static ImportedCityObject[] AlignTerrainGridChunkBoundaries(
        IReadOnlyList<ImportedCityObject> cityObjects)
    {
        return DemTerrainGridChunkBoundaryAligner.AlignAdjacentBoundaries(cityObjects);
    }

    private static SurfaceGeneratedUvProjection CreateSurfaceGeneratedUvProjectionForTest(
        ...
        FacadeUvProjectionContext? context = string.Equals(packageName, "bldg", StringComparison.Ordinal)
            || string.Equals(packageName, "ubld", StringComparison.Ordinal)
                ? new FacadeUvProjectionContext(
                    ...
                    floorCount)
                : null;
        return SurfaceGeneratedUvProjection.TryCreate(surface, packageName, cityObjectOrigin, cartesian, context)
            ?? throw new InvalidOperationException("SurfaceGeneratedUvProjection returned null.");
    }
        ...
        IReadOnlyList<GeodeticPoint> vertices)
    {
        return new ParsedSurface(
            polygonId,
            semantic,
            new ParsedRing($"{polygonId}-ring", vertices.ToArray(), null),
            [],
    ...
    private static HashSet<string> GetBottomBandCulledSurfaceIdsForTest(
        string packageName,
        IEnumerable<ParsedSurface> surfaces,
        GeodeticPoint cityObjectOrigin,
        GeographicLib.LocalCartesian cartesian)
    {
        return BottomBandSurfaceCuller.GetCulledSurfaceIds(
            packageName,
            surfaces,
            cityObjectOrigin,
            cartesian);
    ...
    private static MaterialBinding[] CreateSharedCommonMaterialBindings(
        ParsedCityObject cityObject,
        GeodeticPoint cityObjectOrigin,
        GeographicLib.LocalCartesian cartesian)
    {
        return CommonMaterialBindingEnumerator.CreateSharedBindings(
            cityObject,
            cityObjectOrigin,
            cartesian,
            demTerrainTextureOverlay: null,
    ...
    private static ImportedCityObject ProjectTerrainMeshModeForTest(
        ParsedCityObject cityObject,
        ...
        PlateauImportRequest request)
    {
        ...
        return TerrainMeshModeProjector.Project(
            cityObject,
            globalOriginPoint,
            globalCartesian: null,
            demTerrainTextureOverlay: null,
            ...
            requestedMeshAreas,
            new DefaultMaterialResolver(CommonMaterialCatalog.Create()),
            LocalCityGmlObjectProjection.ProjectCityObject,
