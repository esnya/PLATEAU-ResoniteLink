...
- Bundled default material textures under `src/PlateauResoniteLink.Resonite/Assets/DefaultMaterials/` are grouped by source site or generation source. AmbientCG assets are tracked in `THIRD_PARTY_LICENSES/ambientCG-CC0-1.0.txt`, TextureCan assets are tracked in `THIRD_PARTY_LICENSES/texturecan-CC0-1.0.txt`, and generated WallSkin fallback textures are tracked in `THIRD_PARTY_LICENSES/WallSkin-Generated-Textures.txt`.
- The default DEM terrain imagery overlay is not bundled. It is generated from the PLATEAU ortho tile endpoint `https://api.plateauview.mlit.go.jp/tiles/plateau-ortho-2023/{z}/{x}/{y}.png`, with GSI seamless photo fallback `https://cyberjapandata.gsi.go.jp/xyz/seamlessphoto/{z}/{x}/{y}.jpg` unless `--exclude-gsi-terrain-tiles` is specified. Repository-local notes for the fallback source live in `THIRD_PARTY_LICENSES/gsi-seamlessphoto.txt`.
